package com.test.movies.refactor;
...
public class ProductDetailActivity extends BaseFragmentActivity implements
    ...
    private final int PRODUCTS_TO_SEARCH = 30;

    private final int PRODUCTO_TO_SEARCH_ONE = 1;
    ...
    private void startTutorial() {
        ...
        if (!ManagerPreferences.get().getBoolean(NewWorldPreferencesKey.DETAIL_PRODUCT_TUTORIAL)) {
            startActivityForResult(new Intent(this, TutorialActivity.class)
                    .putExtra(TutorialActivity.TUTORIALS, TutorialActivity.DETAIL_PRODUCT_TUTORIAL), PRODUCT_TUTORIAL_REQUEST);
        } else {
            initProducts();
        }
    }
    ...
    private void initProducts() {
        ...
        manageDetailProductBehavior(newWorldProduct, PRODUCTS_TO_SEARCH, false);
    }

    private void manageDetailProductBehavior(NewWorldProduct newWorldProduct, int productsToSearch, boolean updateView) {

        if(isMultipleDetail){
            if(mainViewModel.isInitSeekerProductDetail() || mainViewModel.isInitTrolleyProductDetail())
    ...
    private void launchProductDetailFromStore(NewWorldProduct newWorldProduct, int productsToSearch, boolean updateView) {
        new LoadDetailProducts(mainViewModel.getGlobalIndexes(String.valueOf(newWorldProduct.getLinearProductIndex()), productsToSearch))
                .execute(newWorldProducts -> {
    ...
    private void launchMultipleProductDetail(boolean updateView) {
        if (ArrayUtil.collectionIsNotEmpty(mainViewModel.getCarouselDetailProducts())) {
            updateView(mainViewModel.getCarouselDetailProducts(), updateView);
        }
    }

    private void launchSingleProductDetail(boolean updateView) {
        updateView(Collections.singletonList(newWorldProduct), updateView);
    }
    ...
    private void initView(ArrayList<NewWorldProduct> newWorldProducts) {
        ...
        productsPagerAdapter = new ProductDetailAdapter(getSupportFragmentManager(), newWorldProducts, getIntent().getExtras().getBoolean(FROM_LIST, false));
        LogUtil.e(TAG, "Size adapter: " + productsPagerAdapter.getCount());
    }
    ...
    private int getPagerPosition(ArrayList<NewWorldProduct> newWorldProducts) {
        ...
        for (int i = 0; i < newWorldProducts.size(); i++) {
            nwp = newWorldProducts.get(i);
            if (nwp.getId().equalsIgnoreCase(this.newWorldProduct.getId()) &&
                    nwp.getLinearProductIndex() == this.newWorldProduct.getLinearProductIndex())
                currentPosition = i;
        }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if(resultCode == RESULT_OK){
            ...
            else if(requestCode == BASCULE_REQUEST){
            ...
            }else if(requestCode == COMMENT_FRAGMENT_REQUEST)
                productsPagerAdapter.getCurrentFragment().addCommentFragment();
        }else{
            if (requestCode == PRODUCT_TUTORIAL_REQUEST){
    ...
    public void onSwipeAtRight(NewWorldProduct newWorldProductId) {
        ...
        manageDetailProductBehavior(newWorldProductId, PRODUCTO_TO_SEARCH_ONE, true);
    }
    ...
    public void onPageScrollStateChanged(int state) {
        if (storeDetailProduct) {
            int lastPage = productsPagerAdapter.getCount();
            if ((previousState == ViewPager.SCROLL_STATE_SETTLING
                    && state == ViewPager.SCROLL_STATE_IDLE)
                    && pagerPosition == (lastPage - PAGE)) {
                goToProduct(productsPagerAdapter.getNewWorldProduct(pagerPosition - PAGE));
            }
            previousState = state;
        }
    }
